...
# Some OpenAI-compatible gateways need endpoint-specific handling (model-name
# aliasing, a tighter context window, and a token-refresh hook). The gateway is
# identified by a substring of its base URL, supplied via the environment so no
# site hostname is baked into this release.
#   OPENAI_GATEWAY_HOST_MATCH   substring identifying the gateway (optional)
#   OPENAI_GATEWAY_ALIAS_MATCH  substring identifying a deployment that requires
#                               provider-free model aliases (optional)
_GATEWAY_HOST_MATCH = os.environ.get("OPENAI_GATEWAY_HOST_MATCH", "").strip()
_GATEWAY_ALIAS_MATCH = os.environ.get("OPENAI_GATEWAY_ALIAS_MATCH", "").strip()


def _is_managed_gateway(base_url: str | None) -> bool:
    """True when base_url matches the configured OpenAI-compatible gateway."""
    if not _GATEWAY_HOST_MATCH:
        return False
    return _GATEWAY_HOST_MATCH in str(base_url or "")


# Deployments that expose provider-free model names ("gpt-oss-120b" rather than
# "openai/gpt-oss-120b"). Only applied when OPENAI_GATEWAY_ALIAS_MATCH is set and
# matches the client's base URL.
_PROVIDER_FREE_MODEL_ALIASES = {
    "openai/gpt-oss-120b": "gpt-oss-120b",
    "gpt-oss-120b-131072": "gpt-oss-120b",
    "google/gemma-4-31B-it": "gemma-4-31B-it",
}


def _normalize_gateway_model_name(model: str | None, *, base_url: str | None) -> str:
    """Strip the provider prefix for deployments that require the bare alias."""
    raw_model = str(model or "")
    if not _GATEWAY_ALIAS_MATCH:
        return raw_model
    if _GATEWAY_ALIAS_MATCH not in str(base_url or ""):
        return raw_model
    return _PROVIDER_FREE_MODEL_ALIASES.get(raw_model, raw_model)


def _is_gateway_auth_error(error: Exception, *, base_url: str | None) -> bool:
    """True for gateway messages that indicate an expired/invalid access token."""
    if not _is_managed_gateway(base_url):
        return False
...
def _refresh_gateway_access_token(client_args: Dict, logger_obj=None) -> bool:
    """Refresh a short-lived gateway access token.

    The refresh mechanism is site-specific, so this release does not ship one.
    Set OPENAI_TOKEN_REFRESH_CMD to a shell command that prints a fresh access
    token on stdout and exits 0; it is invoked when the gateway reports an
    expired token. If the variable is unset, the request simply fails and the
    caller is expected to re-authenticate out of band.
    """
    refresh_cmd = os.environ.get("OPENAI_TOKEN_REFRESH_CMD", "").strip()
    if not refresh_cmd:
        if logger_obj is not None:
            logger_obj.warning(
                "[GATEWAY AUTH] Access token appears expired and "
                "OPENAI_TOKEN_REFRESH_CMD is not set; cannot refresh automatically."
            )
        return False
    ...
    if completed.returncode != 0:
        if logger_obj is not None:
            logger_obj.warning(
                "[GATEWAY AUTH] Token refresh failed: returncode=%s stderr=%s",
    ...
    if not token:
        if logger_obj is not None:
            logger_obj.warning("[GATEWAY AUTH] Token refresh returned an empty access token.")
        return False
    ...
    if logger_obj is not None:
        logger_obj.info("[GATEWAY AUTH] Refreshed OpenAI-compatible endpoint access token.")
    return True
...
def _collapse_consecutive_same_role(messages: List[dict]) -> List[dict]:
    ...
    roles must alternate ...`, which some gateways re-wrap as an HTTP
    500. The retry wrapper doesn't classify those as
...
def _choose_base_url(model: str, default_base_url: str) -> str:
    ...
    OPENAI_BASE_URL_GEMMA_FORCE explicitly routes every Gemma call from the
    current process to one endpoint while non-Gemma models retain the default.
    This is the reproducible option for endpoint-stratified experiments.
    ...
    Useful when a deployment serves the actor model on two backends but the
    evaluator model on only one of them, so traffic cannot simply be pointed at
    the second backend. Per-worker determinism (same PID -> same endpoint for
    the worker's lifetime) keeps vLLM prefix-cache hit rates high.
    ...
    forced_url = os.environ.get("OPENAI_BASE_URL_GEMMA_FORCE", "").strip()
    if forced_url and is_gemma:
        return forced_url
    dev_url = (
...
if not _MONITOR_LOG.handlers:
    fh = logging.FileHandler("api_rate_monitor.log")
...
def _is_transient_error(err) -> bool:
    ...
    if _APITimeoutError is not None and isinstance(err, _APITimeoutError):
        return True
    # Treat 5xx as transient -- common when a gateway wraps an upstream vLLM
    # error (e.g. a 400 BadRequest surfaces as a 500).
...
def _client_needs_bare_model_name(client) -> bool:
    """True when this client's deployment requires provider-free model names.

    Some deployments accept both "openai/gpt-oss-120b" and "gpt-oss-120b";
    others accept only the bare alias. Controlled by OPENAI_GATEWAY_ALIAS_MATCH
    so no site URL is hardcoded here.
    """
    if not _GATEWAY_ALIAS_MATCH:
        return False
    try:
        return _GATEWAY_ALIAS_MATCH in str(getattr(client, "base_url", "") or "")
    except Exception:
        return False


def _maybe_rewrite_model_for_endpoint(client, kwargs):
    """Strip the 'openai/' provider prefix when the deployment requires it."""
    if not _client_needs_bare_model_name(client):
        return kwargs
...
class OpenAIChat(BaseChatModel):
    ...
    def send_token_limit(self, model: str, base_url: str | None = None) -> int:
        send_token_limit_dict = {
            ...
            # Some deployments expose the provider-free alias only.
        ...
        if _is_managed_gateway(base_url) and model in {
            "meta-llama/Meta-Llama-3.1-70B-Instruct",
            "meta-llama/Meta-Llama-3.1-8B-Instruct",
        }:
            # Some gateway deployments enforce a 16k total window for these
            # Llama 3.1 instruct endpoints even though the generic model-family
            # default is larger. Applied only for the configured gateway.
    ...
    def _safe_request_args(self, messages: List[dict]) -> Dict:
        ...
        model = _normalize_gateway_model_name(
    ...
    def generate_response(
        ...
        for auth_retry in range(2):
            ...
            try:
                ...
                if (
                    auth_retry == 0
                    and _is_gateway_auth_error(error, base_url=self.client_args.get("base_url"))
                    and _refresh_gateway_access_token(self.client_args, logger_obj=_MONITOR_LOG)
                ):
                    logger.warning(
                        "Gateway auth error detected in sync request; refreshed token and retrying once."
                    )
                    continue
                raise
            ...
            async with client_instance as async_openai_client:
                try:
                    ...
                    if (
                        ...
                        logger.warning(
                            "Gateway auth error detected in async request; refreshed token and retrying once."
                        )
                        continue
                    raise
